# SWAGGER Minimal: Python軽量版実装（Step1 + Step3）

このプロジェクトは、SWAGGERアルゴリズムの**最小限の機能**（Step1前処理 + Step3境界サンプリング）を統合した、軽量で簡単に使えるグラフ生成ツールです。

> **注**: 全ステップの完全実装は親ディレクトリの `swagger/` パッケージを参照してください。
...
| プロジェクト | ステップ | 用途 |
|------------|---------|------|
| **swagger-minimal**（このディレクトリ） | Step1 + Step3 | Python軽量版・学習用 |
| **swagger** | 全6ステップ | Python完全版・本番用 |
| **swagger-minimal-cpp** | Step1 + Step3 | C++軽量版 |
| **swagger-cpp** | 全6ステップ | C++完全版・Python比較用 |
